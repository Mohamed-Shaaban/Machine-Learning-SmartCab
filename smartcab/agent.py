...
#             print(np.finfo(float).eps)
        ...
        if testing:
            ...
            self.tolerance+=1.0
            #self.alpha = round(random.uniform (0.0,1.0),2) 
#             self.tolerance = round(random.uniform (0.0,1.0),2)
            
#             self.alpha = round(random.uniform (0.0,1.0),2)             
            self.epsilon =math.fabs(math.cos(self.alpha*self.tolerance))
            #self.epsilon = 1.0/(self.tolerance**2)# ZeroDivisionError: 
    ...
    def build_state(self):
        ...
        Keys = ['light', 'oncoming', 'left']
        state = tuple(inputs[key] for key in Keys)
        state += (waypoint,)
        return state
    #         if self.learning:
    ...
    def choose_action(self, state):
        ...
        if not self.learning or random.random() < self.epsilon:
            ...
            Qs = [] # build list of actions that match the max Q value
            for act in self.Q[state]:
                if self.Q[state][act] == maxQ:
                    Qs.append(act)
            action = random.choice(Qs) # choose one randomly
